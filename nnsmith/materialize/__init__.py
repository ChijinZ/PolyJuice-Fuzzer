...
class Model(ABC):
    ...
    @abstractmethod
    def make_oracle(self, input_dic: Dict = None) -> Oracle:
        pass
...
class Symptom(Enum):
    ...
    EQ_INCONSISTENCY = "eq_inconsistency"
    # PARTIAL_EQ_INCONSISTENCY = "partial_eq_inconsistency"
    EQ_IS_WRONG = "eq_is_wrong"
    DOUBLE_INCONSISTENCY = "double_inconsistency"
...
class BugReport(ABC):
    def __init__(
            ...
            stage: Stage,
            system: str,
            eq_testcase: Optional[TestCase] = None,
            version: str = None,
            version_id: str = None,
            log: str = None,
    ...
    def dump(self, root_folder: PathLike):
        ...
        if self.eq_testcase is None:
            # model*
            # oracle.pkl
            self.testcase.dump(root_folder)
        else:
            testcase1_path = os.path.join(root_folder, "t1")
            testcase2_path = os.path.join(root_folder, "t2")
            if not os.path.exists(testcase1_path):
                os.makedirs(testcase1_path)
            if not os.path.exists(testcase2_path):
                os.makedirs(testcase2_path)
            self.testcase.dump(testcase1_path)
            self.eq_testcase.dump(testcase2_path)
    ...
    def load(model_type, root_folder: str, allow_partial=False) -> "BugReport":
        ...
        if os.path.exists(os.path.join(root_folder, "meta.json")):
            ...
            if "other_info" in meta:
                other_info = meta["other_info"]
        ...
        testcase1_path = os.path.join(root_folder, "t1")
        testcase2_path = os.path.join(root_folder, "t2")
        if os.path.exists(testcase1_path) and os.path.exists(testcase2_path):
            testcase1 = TestCase.load(model_type, testcase1_path, allow_partial)
            testcase2 = TestCase.load(model_type, testcase2_path, allow_partial)
            return BugReport(testcase=testcase1,
            ...
            return BugReport(testcase=testcase,
                             eq_testcase=None,
                             symptom=symptom,
                             stage=stage,
                             system=system,
...
"""
    ...
    def emit_input(self, model: Model, path: Optional[PathLike] = None, input_dict: Dict = None):
        # Load the model weights from ${path}
        self.input_code = model.emit_input(inp_name=self.inp_name, path=path, input_dict=input_dict)
