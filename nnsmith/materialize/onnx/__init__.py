...
class ONNXModel(TorchModel):
    ...
    @property
    def with_symbol(self) -> bool:
        return isinstance(self.torch_model, SymbolNet)
    ...
    def gir(self) -> GraphIR:
        if self.with_symbol:
            ...
            return self.torch_model.torch_model.ir
    ...
    def get_onnx_from_torch(self) -> onnx.ModelProto:
        ...
        if self.with_symbol:
            ...
            torch2onnx(self.torch_model.torch_model, f)
    ...
    def emit_def(self, mod_name: str, mod_cls: str) -> str:
        if self.with_symbol:
            s = super().emit_def(mod_name, mod_cls)
        else:
            s = self.torch_model.emit_def(mod_name, mod_cls)
        # s += f"{mod_name} = torch.onnx. {mod_name}\n"
        return s

    def make_oracle(self, input_dic_: Dict = None) -> Oracle:
        if self.with_symbol:
            o = super().make_oracle(input_dic_)
        else:
            o = self.torch_model.make_oracle(input_dic_)
        return o

    def make_weights_consist_with_other(self, other_model: Model):
        assert isinstance(other_model, ONNXModel)
        other_torch_model = other_model if other_model.with_symbol else other_model.torch_model
        
        if self.with_symbol:
            super().make_weights_consist_with_other(other_torch_model)
        else:
            self.torch_model.make_weights_consist_with_other(other_torch_model)
    ...
    def corresponding_input_map(self, other_model: Model) -> Dict[str, str]:
        ...
        symbol_net_model = self.torch_model if self.with_symbol else self.torch_model.torch_model
        other_symbol_net_model = other_model.torch_model if other_model.with_symbol else other_model.torch_model.torch_model
        
        assert (symbol_net_model.input_name_map.keys()) == \
               set(other_symbol_net_model.input_name_map.keys())
        name_map = {}
        for op_index in symbol_net_model.input_name_map.keys():
            self_input_name = symbol_net_model.input_name_map[op_index]
            other_input_name = other_symbol_net_model.input_name_map[op_index]
