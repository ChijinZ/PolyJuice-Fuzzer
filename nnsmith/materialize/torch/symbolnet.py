...
from nnsmith.abstract.op import AbsOpBase, ConcreteOp, Input, Constant
...
def random_tensor(shape, dtype: torch.dtype, margin=4, base=5, use_cuda: Optional[torch.device] = None):
...
class SymbolNet(nn.Module):
    def __init__(
        ...
        self._device = None
        self.ir = ir
        self.special_op: Dict[int, nn.Module] = {}
        ...
        for i, inst in enumerate(self.ir.insts):
            if not isinstance(inst.iexpr.op, Input):
                ...
                if isinstance(target, nn.Parameter):
                    assert isinstance(inst.iexpr.op, Constant), inst.iexpr.op
                ...
                if "op_index" in inst.iexpr.op.extra_attrs:
                    self.special_op[inst.iexpr.op.extra_attrs["op_index"]] = target
            else:
                if "op_index" in inst.iexpr.op.extra_attrs:
    ...
    def grad_input_gen(
            ...
            use_cuda: Optional[torch.device] = None,
            max_time=None,
            **kwargs,
        ...
        if use_cuda:
            for k in inputs:
                inputs[k] = inputs[k].cuda(device=use_cuda)
    ...
    def forward(self, *args):
        ...
        for stmt_idx, (inst, inps, outs, op) in enumerate(self.instructions):
            ...
            elif isinstance(output_tensors, tuple):
