...
class Split(AbsOpBase):
    MAX_ARITY = 5
    MAX_RANK = 5
    in_dtypes = [(i,) for i in DTYPE_GEN_ALL]

    def __str__(self) -> str:
        return "Split" + str(self.arity) + " " + str(self.extra_attrs).replace(":", "=")

    def __init__(self, arity):
        super().__init__()
        SanityCheck.le(arity, Split.MAX_ARITY)
        self.arity = arity
        self.inp_ranks = [(rank_from(1))]
        self.out_ranks = [(rank_from(1))] * arity

    def _init_split_axis(self, input_shapes: List[AbsTensor]) -> int:
        if "axis" not in self.extra_attrs:
            self.extra_attrs["axis"] = random.randint(0, input_shapes[0].ndims - 1)
        return self.extra_attrs["axis"]
    ...
    def requires(self, input_shapes: List[AbsTensor]) -> List[Union[z3.BoolRef, bool]]:
        ndims = input_shapes[0].ndims
        axis = self._init_split_axis(input_shapes)
        SanityCheck.gt(ndims, axis)

        return [nnsmith_eq(nnsmith_mod(input_shapes[0].shape[axis], self.arity), 0)]

    def type_transfer(self, input_shapes: List[AbsTensor]) -> List[AbsTensor]:
        SanityCheck.true(input_shapes[0].ndims > 0)
        axis = self._init_split_axis(input_shapes)
        os = AbsTensor(input_shapes[0].shape, input_shapes[0].dtype)
        os.shape[axis] = nnsmith_div(os.shape[axis], self.arity)
        return [os] * self.arity
