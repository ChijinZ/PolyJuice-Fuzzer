...
from nnsmith.abstract.op import (
    ...
    Placeholder,
    concretize_op,
    Conv1d, NCHWConv2d, NearestInterp, LinearInterp, BilinearInterp, BicubicInterp, TrilinearInterp, Split2, Clip
)
...
def infer_topset_from_scratch(
    ...
    if op_types is None:
        op_types_ = model_cls.operators()
    op_types = []
    sys_name = factory.system_name
    if sys_name == "infinitetensor":
        # fault: Clip(cuda)
        # error: Squeeze, Conv1d, TorchReduceSum, ReduceMean, Linear, Cast, PTMatMul, Split
        # stuck: Interp
        filter_ops = [NearestInterp, LinearInterp, BilinearInterp, BicubicInterp, TrilinearInterp, Split2,]
        if factory.target == "cuda":
            filter_ops.append(Clip)
        for x in op_types_:
            if x not in filter_ops:
                op_types.append(x)
    elif factory.target == "cuda" and (sys_name == "tensorrt"  or sys_name == "pt2"):
        for x in op_types_:
            if x != Conv1d and x != NCHWConv2d:
                op_types.append(x)
    else:
        op_types = op_types_
