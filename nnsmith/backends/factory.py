...
class RedirectStderrToFile:
    ...
    def __enter__(self):
        ...
        self.new_stderr_fd = self.log_file.fileno()
        self.saved_stderr_fd = os.dup(self.orig_stderr_fd)
        os.dup2(self.new_stderr_fd, self.orig_stderr_fd)
...
class BackendFactory(ABC):
    ...
    def checked_compile_and_exec(
            self, testcase: TestCase, crash_safe=False, timeout=None, file_path_for_subprocess_log=None
    ) -> Union[Dict[str, np.ndarray], BugReport]:
        ...
        with mp.Manager() as manager:
            ...
            def crash_safe_compile_exec_wrapper(sdict, log_path):
                if log_path is None:
                    crash_safe_compile_exec(sdict)
                else:
                    with RedirectStderrToFile(log_path):
                        crash_safe_compile_exec(sdict)
            ...
            p = mp.Process(target=crash_safe_compile_exec_wrapper, args=(shared_dict, file_path_for_subprocess_log))
    ...
    def init(
        ...
        if name == "onnxruntime":
            ...
            from nnsmith.backends.hidet import Hidet

            return Hidet(target=target, optmax=optmax, ad=ad, **kwargs)
        elif name == "infinitensor":
            from nnsmith.backends.infinitensor import IT

            return IT(target=target, optmax=optmax, **kwargs)
        else:
