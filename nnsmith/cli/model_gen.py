...
from equality_saturation_helper.helper import EquivalentGraphHelper, FFIHelper
...
def main(cfg: DictConfig):
    for _ in range(1):
        ...
        MGEN_LOG.info(ir.to_dot())

        ffi_helper = FFIHelper()
        helper = EquivalentGraphHelper(ffi_helper, ir)
        helper.initialize_inner_graph()
        helper.test_helper_lib()
        ir2 = helper.output_inner_graph()
        ...
        mkdir(mgen_cfg["save"], yes="y")
        if cfg["debug"]["viz"]:
            fmt = cfg["debug"]["viz_fmt"].replace(".", "")
            viz(ir, os.path.join(mgen_cfg["save"], f"graph.{fmt}"))

        model1 = ModelType.from_gir(ir)
        model1.refine_weights()  # either random generated or gradient-based.
        model1.set_grad_check(mgen_cfg["grad_check"])
        model2 = ModelType.from_gir(ir2)
        model2.refine_weights()
        model2.set_grad_check(mgen_cfg["grad_check"])

        oracle = model2.make_oracle()
        tmat = time.time() - tmat_begin

        tsave_begin = time.time()
        testcase = TestCase(model1, oracle)
