...
class BaseGen:
    ...
    def backward_insert_node(
        ...
        # and preserve ph for multiple outputs.
        ph_next_list = ph_to_replace[1:] + [None]
        # for ph, rv in zip(ph_to_replace, new_inst.retvals()):
        for ph, rv, ph_next in zip(ph_to_replace, new_inst.retvals(), ph_next_list):
    ...
    def try_insert_node_type(
        ...
        try:
            for _ in range(max_tensor_pick_time):
                ...
                if issubclass(type(op), Split) or random.uniform(0, 1) < self.forward_prob:
    ...
    def eliminate_extra_outputs(self):
        ...
        while prev_size != self.ir.n_inst():
            ...
            for cut_index in range(len(cuts) - 1, -1, -1):
                cut = cuts[cut_index]
                if len(cut[0].retvals()) == 1:
                    selected_cut_index = cut_index
                    break
            if selected_cut_index is None:
                raise BaseException
            for cut_index, cut in enumerate(cuts):
                if cut_index == selected_cut_index:
                    continue
                for inst in cut:
                    self.ir.remove_unused(inst)
            self.ir.assert_wellform()
        SanityCheck.eq(len(self.ir.leaf_var()), 1, f"Failed to eliminate extra outputs!, {self.ir}")
